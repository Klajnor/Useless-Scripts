;with c as (
  select
    ...
    case when datediff(d, c.datebeg, convert(date, getdate())) in (0, -1, -2, -3) then 0 else 1 end + case when exists(select 'x' from replsentclaim rs where rs.claim = ric.claim) then 1 else 0 end [priority]
